...
echo "nix@nix-build-server x86_64-linux $DIST/keys/id_rsa 1 1" > ~/.nix-remote-systems.conf
...
cp $DIST/keys/id_rsa /etc/nix/signing-key.sec
cp $DIST/keys/signing-key.pub /etc/nix/signing-key.pub
mkdir -p /tmp/current-load
